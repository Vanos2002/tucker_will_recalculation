...
// RK4 adaptive integrator for QLT equations
struct RK4QLTStep {
    double p, alpha, beta, theta;
    int n_function_evals;
};

RK4QLTStep rk4_qlt_adaptive_step(const PNCoeffs& K, double p, double alpha, double beta, double theta,
                                 double dp_target, double c_val, int PNorder, double& dp_actual)
{
    const int avg_samples = 128;
    const double tol = 1e-6;
    double angle = atan2(beta, alpha);
    
    double dp = dp_target;
    double h_min = dp_target * 0.01;  // Don't go below 1% of target step
    int n_evals = 0;
    
    for (int attempt = 0; attempt < 10; ++attempt) {
        // Full step
        auto q1 = orbitAverage(K, p, alpha, beta, c_val, PNorder, avg_samples);
        double dtheta_dp1 = 1.0 / (fabs(q1.dp) < 1e-18 ? 1e-18 : q1.dp);
        double e = sqrt(alpha * alpha + beta * beta);
        n_evals++;
        
        auto q2 = orbitAverage(K, p + 0.5*dp, alpha + 0.5*q1.de*dtheta_dp1*dp, 
                               beta + 0.5*q1.de*dtheta_dp1*dp, c_val, PNorder, avg_samples);
        double dtheta_dp2 = 1.0 / (fabs(q2.dp) < 1e-18 ? 1e-18 : q2.dp);
        n_evals++;
        
        auto q3 = orbitAverage(K, p + 0.5*dp, alpha + 0.5*q2.de*dtheta_dp2*dp,
                               beta + 0.5*q2.de*dtheta_dp2*dp, c_val, PNorder, avg_samples);
        double dtheta_dp3 = 1.0 / (fabs(q3.dp) < 1e-18 ? 1e-18 : q3.dp);
        n_evals++;
        
        auto q4 = orbitAverage(K, p + dp, alpha + q3.de*dtheta_dp3*dp,
                               beta + q3.de*dtheta_dp3*dp, c_val, PNorder, avg_samples);
        double dtheta_dp4 = 1.0 / (fabs(q4.dp) < 1e-18 ? 1e-18 : q4.dp);
        n_evals++;
        
        // RK4 update
        double theta_new = theta + (dp/6.0) * (dtheta_dp1 + 2*dtheta_dp2 + 2*dtheta_dp3 + dtheta_dp4);
        double de_new = (1.0/6.0) * (q1.de*dtheta_dp1 + 2*q2.de*dtheta_dp2 + 2*q3.de*dtheta_dp3 + q4.de*dtheta_dp4) * dp;
        double e_new = e + de_new;
        
        // Error estimate using half-step comparison (simplified)
        double error = fabs(de_new) * 0.01;  // Rough error estimate
        
        if (error < tol || dp <= h_min) {
            double alpha_new = e_new * cos(angle);
            double beta_new = e_new * sin(angle);
            dp_actual = dp;
            return {p + dp, alpha_new, beta_new, theta_new, n_evals};
        }
    ...
    dp_actual = dp;
    return {p + dp, alpha_new, beta_new, theta + 0.001, n_evals};
}
                                     ...
                                     int PNorder = 4)
{
    ...
    double target_step = dp_total / 100.0;  // Target ~100 steps
    int n_steps = 0;
    
    while ((dp_total > 0 && p < p_final) || (dp_total < 0 && p > p_final)) {
        double remaining = p_final - p;
        double dp_step = (fabs(remaining) < fabs(target_step)) ? remaining : 
                         (remaining > 0 ? target_step : -target_step);
        
        double dp_actual;
        auto result = rk4_qlt_adaptive_step(K, p, alpha, beta, theta, dp_step, c_val, PNorder, dp_actual);
        ...
        if (!isfinite(theta) || !isfinite(alpha) || !isfinite(beta) || n_steps > 100000) {
            break;
        }
    }
                            ...
                            double c_val, double eta)
{
    ...
    while ((dp_total > 0 && p < p_final) || (dp_total < 0 && p > p_final)) {
        ...
        double dp_actual;
        auto result = rk4_tw_adaptive_step(p, e, theta, dp_step, c_val, dp_actual);
        ...
        if (!isfinite(theta) || !isfinite(e) || n_steps > 100000) {
            break;
        }
    }
...
// RK4 adaptive integrator for Fereisl equations
struct RK4JFStep {
    double p, e, theta;
    int n_function_evals;
};

RK4JFStep rk4_jf_adaptive_step(double p, double e, double theta, double dp_target, double& dp_actual)
{
    const double tol = 1e-6;
    double dp = dp_target;
    double h_min = dp_target * 0.01;
    ...
    for (int attempt = 0; attempt < 10; ++attempt) {
        // RK4 for JF
        double dp_dth1 = dp_JF(p, e);
        double de_dth1 = de_JF(p, e);
        double dth_dp1 = 1.0 / (fabs(dp_dth1) < 1e-16 ? 1e-16 : dp_dth1);
        n_evals++;
        
        double dp_dth2 = dp_JF(p + 0.5*dp, e + 0.5*de_dth1*dth_dp1*dp);
        double de_dth2 = de_JF(p + 0.5*dp, e + 0.5*de_dth1*dth_dp1*dp);
        double dth_dp2 = 1.0 / (fabs(dp_dth2) < 1e-16 ? 1e-16 : dp_dth2);
        n_evals++;
        
        double dp_dth3 = dp_JF(p + 0.5*dp, e + 0.5*de_dth2*dth_dp2*dp);
        double de_dth3 = de_JF(p + 0.5*dp, e + 0.5*de_dth2*dth_dp2*dp);
        double dth_dp3 = 1.0 / (fabs(dp_dth3) < 1e-16 ? 1e-16 : dp_dth3);
        n_evals++;
        
        double dp_dth4 = dp_JF(p + dp, e + de_dth3*dth_dp3*dp);
        double de_dth4 = de_JF(p + dp, e + de_dth3*dth_dp3*dp);
        double dth_dp4 = 1.0 / (fabs(dp_dth4) < 1e-16 ? 1e-16 : dp_dth4);
        n_evals++;
        
        double theta_new = theta + (dp/6.0) * (dth_dp1 + 2*dth_dp2 + 2*dth_dp3 + dth_dp4);
        double de_new = (1.0/6.0) * (de_dth1*dth_dp1 + 2*de_dth2*dth_dp2 + 2*de_dth3*dth_dp3 + de_dth4*dth_dp4) * dp;
        double e_new = e + de_new;
...
IntegrationResult integrateJF(double p_init, double p_final, 
    ...
    while ((dp_total > 0 && p < p_final) || (dp_total < 0 && p > p_final)) {
        ...
        double dp_actual;
        auto result = rk4_jf_adaptive_step(p, e, theta, dp_step, dp_actual);
