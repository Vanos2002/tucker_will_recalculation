...
static double richardsonExtrapolate(double coarse, double fine) {
    return (4.0 * fine - coarse) / 3.0;
}

static IntegrationResult integrateOscillatoryStep(int n_steps,
                                                 double p_init, double p_final,
                                                 double alpha_t, double beta_t,
                                                 double c_val, double eta,
                                                 int PNorder)
{
    PNCoeffs K = buildCoefficients(true);
    double eps = 1.0 / c_val;
    double p = p_init;
    ...
    double dp_total = p_final - p_init;
    double dp_step = dp_total / n_steps;
    double sign = (dp_step >= 0.0) ? 1.0 : -1.0;
    double angle = atan2(beta, alpha);
    const int averaging_samples = 128;

    for (int step = 0; step < n_steps; ++step) {
        AvgResult avg1 = orbitAverage(K, p, alpha, beta, c_val, PNorder, averaging_samples);
        double dp_dtheta1 = avg1.dp;
        if (fabs(dp_dtheta1) < 1e-18) dp_dtheta1 = sign * 1e-18;
        double de_dtheta1 = avg1.de;
        double dtheta_dp1 = 1.0 / dp_dtheta1;

        double p_mid = p + 0.5 * dp_step;
        double e = sqrt(alpha * alpha + beta * beta);
        double e_mid = e + 0.5 * (de_dtheta1 * dtheta_dp1) * dp_step;
        double alpha_mid = e_mid * cos(angle);
        double beta_mid = e_mid * sin(angle);

        AvgResult avg2 = orbitAverage(K, p_mid, alpha_mid, beta_mid, c_val, PNorder, averaging_samples);
        double dp_dtheta2 = avg2.dp;
        if (fabs(dp_dtheta2) < 1e-18) dp_dtheta2 = sign * 1e-18;
        double de_dtheta2 = avg2.de;
        double dtheta_dp2 = 1.0 / dp_dtheta2;

        theta += dtheta_dp2 * dp_step;
        double e_new = e + (de_dtheta2 * dtheta_dp2) * dp_step;
        alpha = e_new * cos(angle);
        beta = e_new * sin(angle);
        p += dp_step;

        if (!isfinite(theta) || !isfinite(alpha) || !isfinite(beta)) {
            break;
        }
    }
...
{
    double eps = 1.0 / c_val;
    int n_steps = computeOptimalSteps(eps, p_init, p_final);
    int fine_steps = std::min(n_steps * 2, 500000);

    auto coarse = integrateOscillatoryStep(n_steps, p_init, p_final, alpha_t, beta_t, c_val, eta, PNorder);
    auto fine   = integrateOscillatoryStep(fine_steps, p_init, p_final, alpha_t, beta_t, c_val, eta, PNorder);

    double phi_ref = fine.phi_total;
    if (std::isfinite(coarse.phi_total) && std::isfinite(fine.phi_total)) {
        phi_ref = richardsonExtrapolate(coarse.phi_total, fine.phi_total);
    }
    double delta_phi = std::isfinite(phi_ref) ? fabs(phi_ref - fine.phi_total) : fabs(coarse.phi_total - fine.phi_total);

    return {p_final, phi_ref, delta_phi, (double)fine_steps, eps};
}

static double integrateTWStep(int n_steps,
                              double p_init, double p_final,
                              double e_init,
                              double c_val)
{
    ...
    for (int step = 0; step < n_steps; ++step) {
        double x = x_TW(p);
        double dp_dtheta1 = dp_TW_dtheta(e, x);
        double de_dtheta1 = de_TW_dtheta(e, x);
        if (fabs(dp_dtheta1) < 1e-16) dp_dtheta1 = sign * 1e-16;
        double de_dp1 = de_dtheta1 / dp_dtheta1;
        double dtheta_dp1 = 1.0 / dp_dtheta1;

        double p_mid = p + 0.5 * dp_step;
        double e_mid = e + 0.5 * de_dp1 * dp_step;

        double x_mid = x_TW(p_mid);
        double dp_dtheta2 = dp_TW_dtheta(e_mid, x_mid);
        double de_dtheta2 = de_TW_dtheta(e_mid, x_mid);
        if (fabs(dp_dtheta2) < 1e-16) dp_dtheta2 = sign * 1e-16;
        double de_dp2 = de_dtheta2 / dp_dtheta2;
        double dtheta_dp2 = 1.0 / dp_dtheta2;
                            ...
                            double c_val, double eta)
{
    ...
    double e_init = sqrt(alpha_t * alpha_t + beta_t * beta_t);
    double phi_coarse = integrateTWStep(n_steps, p_init, p_final, e_init, c_val);
    double phi_fine   = integrateTWStep(fine_steps, p_init, p_final, e_init, c_val);

    double phi_ref = phi_fine;
    if (std::isfinite(phi_coarse) && std::isfinite(phi_fine)) {
        phi_ref = richardsonExtrapolate(phi_coarse, phi_fine);
    }
    double delta_phi = std::isfinite(phi_ref) ? fabs(phi_ref - phi_fine) : fabs(phi_coarse - phi_fine);

    return {p_final, phi_ref, delta_phi, (double)fine_steps, eps};
}

static double integrateJFStep(int n_steps,
                              double p_init, double p_final,
                              double e_init)
{
    double p = p_init;
    ...
    for (int step = 0; step < n_steps; ++step) {
        double dp_dtheta1 = dp_JF(p, e);
        double de_dtheta1 = de_JF(p, e);
        if (fabs(dp_dtheta1) < 1e-16) dp_dtheta1 = sign * 1e-16;
        double de_dp1 = de_dtheta1 / dp_dtheta1;
        double dtheta_dp1 = 1.0 / dp_dtheta1;
        ...
        double dp_dtheta2 = dp_JF(p_mid, e_mid);
        double de_dtheta2 = de_JF(p_mid, e_mid);
        if (fabs(dp_dtheta2) < 1e-16) dp_dtheta2 = sign * 1e-16;
        double de_dp2 = de_dtheta2 / dp_dtheta2;
        double dtheta_dp2 = 1.0 / dp_dtheta2;
...
IntegrationResult integrateJF(double p_init, double p_final, 
    ...
    double e_init = sqrt(alpha_t * alpha_t + beta_t * beta_t);
    double phi_coarse = integrateJFStep(n_steps, p_init, p_final, e_init);
    double phi_fine   = integrateJFStep(fine_steps, p_init, p_final, e_init);
