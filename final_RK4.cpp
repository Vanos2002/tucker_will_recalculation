...
static double stateStep(double x) {
    return std::max(1e-8, 1e-8 * std::max(1.0, std::abs(x)));
}
                                          ...
                                          const PhysicalParams& params)
{
    ...
    for (int i = 0; i < PHI_AVERAGE_SAMPLES; ++i) {
        ...
        result[0] += (F45.dp + corr1.dp + corr2.dp + corr3.dp);
        result[1] += (F45.dalpha + corr1.dalpha + corr2.dalpha + corr3.dalpha);
        result[2] += (F45.dbeta + corr1.dbeta + corr2.dbeta + corr3.dbeta);
    }
    double scale = h / (2.0 * PI);
    result[0] *= scale;
    result[1] *= scale;
